...
var colors = ['#5061bc', '#e3458a', '#FFB399', '#FF33FF', '#FFFF99', '#00B3E6',
...
function createLabels(labels, old) {


    const labelsLength = labels.length;
    var arr = [];
    for (var i = 0; i < labelsLength; i++) {
        if (!old || (old && !storedNames.includes(labels[i]))) {
            arr.push(<Area key={globalLabelCount} type='monotone' dataKey={labels[i]}
                           fillOpacity={(1 - i % 2 / 1.5)} stroke={colors[i]}
                           fill={colors[i]}/>);
            colors.shift();
            globalLabelCount++;
        }
...
const noRefreshLabels = createLabels(storedNames, false);
...
class SiteVisit extends React.Component {
    ...
    async componentDidMount() {
        ...
        // console.log(siteVisit);
        var labels = createLabels(names, true);
